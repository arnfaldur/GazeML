"""Eyelink frame source for intermediate processing."""
import itertools
import os
...
class EyeLinkFrames(FramesSource):
    """EyeLink frame loading class."""
    ...
    def __init__(self,
        ...
        self._short_name = 'EyeLink_Frames'
        if testing:
        ...
        while True:
            for session in os.scandir(self._dataset_root):
                for sub in sorted(os.scandir(session), key=lambda x: x.path):
                    video_path = sub.path + os.sep + "myndband.avi"
                    if not os.path.isfile(video_path):
                        raise Exception(f"myndband is missing in {sub.path}")
    ...
    def entry_generator(self, yield_just_one=False):
        ...
        while range(1) if yield_just_one else True:
            ...
            with self._proc_mutex:
                ...
                bgr = cv.flip(frame['image'], flipCode=1)  # Mirror
                ...
                self._frames[current_index] = entry
            ...
            self.detect_faces(entry)
            self.detect_landmarks(entry)
            self.calculate_smoothed_landmarks(entry)
            self.segment_eyes(entry)
            self.update_face_boxes(entry)
            entry['time']['after_preprocessing'] = time.perf_counter()

            for i, eye_dict in enumerate(entry['eyes']):
                yield {
                    # 'video_index': np.int64(frame['video_index']),
                    # 'subsession': frame['subsession'],
